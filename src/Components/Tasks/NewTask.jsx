...
const NewTask = ({ setTasks, isCreatingNewTask, setIsCreatingNewTask }) => {
  ...
  const handleOnClick = async () => {
    ...
    const { success, newTask, message } = response;
  ...
  return isCreatingNewTask ?
    ...
    <div className="flex bg-zinc-100 min-w-[200px,20%] justify-between rounded-md border overflow-hidden" onClick={() => inputRef.current.focus()}>
      <div className="cover h-screen w-screen absolute z-[1] top-0 left-0"></div>
      <input
        ref={inputRef}
        className="text-base relative z-[2] w-full px-3 py-2 outline-0"
        type="text"
        value={inputValue}
      ...
      />
      {/* delete button */}
      <div className='cursor-pointer flex items-center hover:bg-zinc-200 relative z-[2] outline' onClick={() => setIsCreatingNewTask(false)}>
