...
const Delete = ({ setCurrentAuthentication }) => {
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex w-full rounded-lg border hover:border-[var(--primary)] overflow-hidden">
        <input
          type={isPasswordHidden ? "password" : "text"}
          value={inputValue}
          onChange={e => setInputValue(e.target.value)}
          className="w-full px-4 py-2 bg-white outline-none border-r hover:border-[var(--primary)]"
          placeholder="Enter Password..."
        />
        <button onClick={() => setIsPasswordHidden(pre => !pre)}>
          {isPasswordHidden ? "Show" : "Hide"}
        </button>
      </div>
      <button className="flex items-center rounded-md w-fit cursor-pointer px-2 py-1 border border-red-500 text-red-500 bg-white" onClick={handleOnClick}>
